...
> Project: Comanda | Type: Full-Stack (Node.js + React) | Version: 1.0.0
...
│   │   │ /mesas     │ │/mozo/nuevo │ │ (keyboard) │ │  pedidos   │       │   │
│   │   │ (grouping) │ │  pedido    │ └────────────┘ └────────────┘       │   │
│   │   │ /productos │ └────────────┘  ┌────────────┐                     │   │
│   │   │ /modific   │                 │   PUBLIC   │                     │   │
│   │   │ /reservas  │                 │ /menu/:slug│ (QR/Web)            │   │
│   │   │ /reportes  │                 └────────────┘                     │   │
│   │   │ /cierre    │  ┌────────────┐                                   │   │
│   │   │ /config    │  │    AUTH    │                                   │   │
│   │   │ /trans-mp  │  │ /registro  │  Keyboard shortcuts: ? for help   │   │
│   │   │ /pedidos   │  │ /verificar │                                   │   │
│   │   │ + más      │  │  email     │                                   │   │
│   │   └────────────┘  └────────────┘                                   │   │
│   │                                                                      │   │
...
│   │   │   │   └── PublicLayout.jsx
│   │   │   ├── ui/
│   │   │   │   └── ShortcutsHelp.jsx # Keyboard shortcuts help modal
│   │   │   └── pedidos/
...
│   │   │   │   ├── Dashboard.jsx
│   │   │   │   ├── Empleados.jsx
│   │   │   │   ├── MesasUnificado.jsx  # Tables: operation + floor plan + grouping
│   │   │   │   ├── Categorias.jsx
│   │   │   │   ├── Productos.jsx
│   │   │   │   ├── Ingredientes.jsx
│   │   │   │   ├── Modificadores.jsx
...
│   │   │   ├── useEventSource.js
│   │   │   ├── useKeyboardShortcuts.js # Reusable keyboard shortcuts
...
│  ├── PATCH  /api/mesas/posiciones         (batch update positions)       │
│  ├── POST   /api/mesas/:id/pedir-cuenta   (changes group to CUENTA_PED) │
│  ├── POST   /api/mesas/grupos             (group 2+ tables together)     │
│  ├── DELETE /api/mesas/grupos/:id         (ungroup tables)               │
│  └── DELETE /api/mesas/:id                                               │
│                                                                            │
│  Productos & Variantes                                                     │
│  ├── GET    /api/productos                                               │
...
│          │           │ capacidad    │  CUENTA_PEDIDA)                       │
│          │           │ grupoMesaId? │ (mesas agrupadas comparten ID)       │
│          │           │ posX?, posY? │ (posición en plano visual)           │
│          │           └──────────────┘                                       │
│          │                                                                   │
│          │ 1:N                                                               │
│          ├───────────────┬───────────────┐                                  │
...
- **Mesa grouping**: `Mesa.grupoMesaId` (nullable Int) groups tables together. Mesas with the same `grupoMesaId` form a group. The value is the ID of the principal mesa (lowest `numero`). Indexed for efficient querying. When all orders in a group are paid, all mesas are freed and `grupoMesaId` is set to `null`.
...
PENDIENTE ──▶ EN_PREPARACION ──▶ LISTO ──▶ ENTREGADO ──▶ COBRADO
    ...
    │              │ (encola impresion)                     │ (si grupo: libera todas
    │              │                                        │  cuando todos cobrados,
    │              │                                        │  y desagrupa)
    └──────────────┴────────── CANCELADO ◀──────────────────┘
                                   │
...
│   ├── mesa.updated      { id, estado, grupoMesaId }                        │
│   ├── pago.updated      { pedidoId, totalPagado, pendiente, estadoPedido }  │
│   ├── cuenta.solicitada { mesaId, numero }                                  │
...
│   ├── MesasUnificado.jsx  → listens: mesa.updated, pedido.updated,          │
│   │                                  reserva.updated, cuenta.solicitada     │
...
| `frontend/src/hooks/useKeyboardShortcuts.js` | Reusable keyboard shortcuts | Adding shortcuts to pages |
| `frontend/src/components/ui/ShortcutsHelp.jsx` | Shortcuts help modal (`?`) | UI for shortcut discovery |
| `frontend/src/pages/admin/MesasUnificado.jsx` | Tables: operation + floor plan + grouping | Mesa changes |
...
# Database (local)
DATABASE_URL="postgresql://user:pass@localhost:5432/comanda?schema=public"
DIRECT_URL="postgresql://user:pass@localhost:5432/comanda?schema=public"
...
- **Mesa grouping**: Mesas with same `grupoMesaId` form a group. The principal mesa is the one with the lowest `numero`. When "Pedir Cuenta" is triggered on a grouped mesa, ALL mesas in the group change to CUENTA_PEDIDA. When all orders in a group are paid, ALL mesas are freed and ungrouped automatically.
- **Keyboard shortcuts**: All main pages support keyboard shortcuts via `useKeyboardShortcuts` hook. Press `?` on any page to see available shortcuts. Shortcuts are ignored when focus is on input/textarea/select (except Escape).
...
|------|-----|--------|
| **Mesas** | `N` | Nueva mesa |
| | `G` | Modo agrupar mesas |
| | `1` / `2` | Tab Operacion / Plano |
| | `Esc` | Cerrar modal / Cancelar agrupar |
| **Pedidos** | `N` | Nuevo pedido |
| | `Esc` | Cerrar modal |
| **Productos** | `N` | Nuevo producto |
| | `Esc` | Cerrar modal |
| **Cierre Caja** | `A` | Abrir caja |
| | `C` | Cerrar caja |
| | `Esc` | Cerrar modal |
| **Cocina** | Flechas / `8-2-4-6` | Navegar pedidos |
| | `Enter` / `5` | Confirmar accion |
| | `0` | Toggle sonido |
| **Global** | `?` | Mostrar/ocultar ayuda de atajos |
