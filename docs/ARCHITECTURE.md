...
│   │       ├── 20260118075557_init/
│   │       ├── 20260118082205_add_delivery_role/
│   │       └── 20260201000000_add_print_jobs/
│   ├── src/
...
│   │   ├── middlewares/
│   │   │   └── auth.middleware.js  # JWT + verificarRol
│   │   ├── services/
│   │   │   ├── email.service.js    # Nodemailer for order confirmations
│   │   │   ├── event-bus.js        # In-memory pub/sub for SSE
│   │   │   └── print.service.js    # Print job queue management
│   │   └── utils/                  # (placeholder)
...
│                                                                            │
│  Pedidos (Orders)                                                          │
│  ├── GET    /api/pedidos             # List orders (filters: estado,tipo) │
│  ├── GET    /api/pedidos/:id         # Get single order                   │
│  ├── GET    /api/pedidos/cocina      # Kitchen view (pending orders)      │
│  ├── GET    /api/pedidos/delivery    # Delivery view                      │
...
│  └── POST   /api/productos/:id/imagen # Upload product image             │
│                                                                            │
│  Inventario                                                                │
...
│  ├── GET    /api/fichajes            # Get employee clock records        │
...
│  └── PATCH  /api/liquidaciones/:id/pagar # Mark as paid                  │
│                                                                            │
│  Pagos                                                                     │
│  ├── POST   /api/pagos               # Register payment                  │
│  ├── GET    /api/pagos/pedido/:id    # List payments for order           │
│  ├── POST   /api/pagos/mercadopago   # Create MP preference              │
│  └── POST   /api/pagos/webhook/mercadopago  # MP webhook (public)        │
│                                                                            │
│  Reportes                                                                  │
│  ├── GET    /api/reportes/dashboard  # Today's metrics                   │
│  ├── GET    /api/reportes/ventas     # Sales report by date range        │
│  ├── GET    /api/reportes/productos-mas-vendidos # Top products          │
│  ├── GET    /api/reportes/ventas-por-mozo # Sales by waiter              │
│  ├── GET    /api/reportes/inventario # Inventory status                  │
│  └── GET    /api/reportes/sueldos    # Payroll report                    │
│                                                                            │
│  Configuración                                                             │
│  ├── GET    /api/configuracion       # Get all config (admin)            │
│  ├── PUT    /api/configuracion/:clave # Update single config             │
│  ├── PUT    /api/configuracion       # Bulk update                       │
│  └── POST   /api/configuracion/banner # Upload banner image              │
│                                                                            │
│  Público (No auth required)                                                │
│  ├── GET    /api/publico/config      # Public store config               │
│  ├── GET    /api/publico/menu        # Public menu (categories+products) │
│  ├── POST   /api/publico/pedido      # Create public order               │
│  ├── GET    /api/publico/pedido/:id  # Get order status                  │
│  └── POST   /api/publico/pedido/:id/pagar  # Init MercadoPago payment   │
│                                                                            │
│  Impresión                                                                 │
│  ├── POST   /api/impresion/comanda/:id  # Enqueue print jobs             │
│  ├── GET    /api/impresion/comanda/:id/preview  # Preview ticket         │
│  ├── GET    /api/impresion/poll      # Poll for pending jobs (bridge)    │
│  └── PATCH  /api/impresion/job/:id   # Update job status                 │
│                                                                            │
│  Eventos (Server-Sent Events)                                              │
│  └── GET    /api/eventos             # SSE stream for real-time updates  │
│                                                                            │
└───────────────────────────────────────────────────────────────────────────┘
```
