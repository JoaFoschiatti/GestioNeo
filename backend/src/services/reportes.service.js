...
const gastosReporte = async (prisma, query) => {
  const { fechaDesde, fechaHasta } = query;
  ...
  for (const mov of movimientos) {
    const cat = mov.categoriaGasto || 'Sin categoria';
    if (!porCategoria[cat]) {
      porCategoria[cat] = { categoria: cat, total: 0, cantidad: 0, movimientos: [] };
    }
    const costo = decimalToNumber(mov.costoTotal);
    porCategoria[cat].total += costo;
    porCategoria[cat].cantidad += 1;
    porCategoria[cat].movimientos.push({
      id: mov.id,
      ingrediente: mov.ingrediente?.nombre,
      unidad: mov.ingrediente?.unidad,
      cantidad: decimalToNumber(mov.cantidad),
      costoUnitario: decimalToNumber(mov.costoUnitario),
      costoTotal: costo,
      fecha: mov.createdAt,
      motivo: mov.motivo
    });
