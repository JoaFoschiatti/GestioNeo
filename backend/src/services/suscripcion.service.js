...
const SUBSCRIPTION_PRICE_USD = parseInt(process.env.SUBSCRIPTION_PRICE_USD || '60', 10);
const SUBSCRIPTION_PRICE_ARS_FALLBACK = parseInt(process.env.SUBSCRIPTION_PRICE_ARS || '72000', 10);
...
function getSaaSMercadoPagoClient() {
  ...
  if (!accessToken) {
    ...
    logger.warn('MP_SAAS_ACCESS_TOKEN no configurado');
    return null;
  }
...
async function getSubscriptionPriceARS() {
  try {
    const config = await prisma.configuracion.findUnique({
      where: { clave: 'cotizacion_usd_ars' }
    });
    if (config) {
      const cotizacion = parseFloat(config.valor);
      if (cotizacion > 0) {
        return { precio: Math.round(SUBSCRIPTION_PRICE_USD * cotizacion), cotizacion };
      }
    }
  } catch (error) {
    logger.error('Error leyendo cotización:', error);
  }
  return { precio: SUBSCRIPTION_PRICE_ARS_FALLBACK, cotizacion: null };
}
...
async function crearSuscripcion() {
  ...
  const { precio: precioARS } = await getSubscriptionPriceARS();
...
async function obtenerEstado() {
  ...
  const { precio: precioARS, cotizacion: cotizacionActual } = await getSubscriptionPriceARS();
  ...
  if (!suscripcion) {
    return {
      ...
      suscripcion: null,
      precio: precioARS,
      precioUSD: SUBSCRIPTION_PRICE_USD,
  ...
  if (suscripcion.mpPreapprovalId) {
    ...
    if (client) {
      try {
        ...
        // eslint-disable-next-line no-console
        logger.error('Error cancelando en MercadoPago:', error);
      }
    }
  }
...
async function procesarWebhook(type, dataId) {
  ...
  if (!client) {
    // eslint-disable-next-line no-console
    logger.error('Webhook de suscripción: MP no configurado');
    return;
  }
...
async function procesarPagoWebhook(client, paymentId) {
  ...
  if (!response.ok) {
    // eslint-disable-next-line no-console
    logger.error('Error obteniendo pago autorizado:', await response.text());
    return;
  }
  ...
  if (!suscripcion) {
    // eslint-disable-next-line no-console
    logger.error('Suscripción no encontrada para pago autorizado');
    return;
  }
