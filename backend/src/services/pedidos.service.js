...
/**
 * Deduce stock usando metodo FIFO cuando hay lotes activos.
 * Consume del lote mas antiguo primero.
 * Retorna movimientos de stock con loteId para trazabilidad.
 */
const deducirStockFIFO = async (tx, ingredienteId, cantidadTotal, pedidoId) => {
  const lotesActivos = await tx.loteIngrediente.findMany({
    where: {
      ingredienteId,
      agotado: false,
      cantidadActual: { gt: 0 }
  ...
  for (const lote of lotesActivos) {
    ...
    const disponible = toNumber(lote.cantidadActual);
    const consumir = Math.min(disponible, restante);

    const nuevaCantidad = disponible - consumir;
    ...
    movimientos.push({
      ...
      tipo: 'SALIDA',
      cantidad: consumir,
      motivo: `Pedido #${pedidoId} (Lote: ${lote.codigoLote || lote.id})`,
      pedidoId,
      loteId: lote.id
    });
  ...
  // Decrementar del pool general (mantener sincronizado)
...
const cambiarEstadoPedido = async (prisma, payload) => {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (shouldPrint) {
      ...
      // Verificar cuales ingredientes tienen lotes activos
      const ingredienteIds = Array.from(ingredienteUpdates.keys());
      const lotesCount = await tx.loteIngrediente.groupBy({
        by: ['ingredienteId'],
        where: {
      ...
      const ingredientesConLotes = new Set(lotesCount.map(l => l.ingredienteId));

      // Separar ingredientes con y sin lotes
      const sinLotes = [];
      const conLotes = [];
      for (const [ingredienteId, cantidadTotal] of ingredienteUpdates.entries()) {
        if (ingredientesConLotes.has(ingredienteId)) {
          conLotes.push({ ingredienteId, cantidadTotal });
        } else {
          sinLotes.push({ ingredienteId, cantidadTotal });
        }
      }

      // Ingredientes SIN lotes: decrement atomico (como antes)
      if (sinLotes.length > 0 || stockMovements.length > 0) {
        await Promise.all([
          ...sinLotes.map(({ ingredienteId, cantidadTotal }) =>
            tx.ingrediente.update({
              where: { id: ingredienteId },
              data: { stockActual: { decrement: cantidadTotal } }
            })
          ),
          // Movimientos de stock de ingredientes sin lotes
          stockMovements.length > 0 ? tx.movimientoStock.createMany({ data: stockMovements }) : Promise.resolve()
        ]);
      }

      // Ingredientes CON lotes: FIFO
      const movimientosFIFO = [];
      for (const { ingredienteId, cantidadTotal } of conLotes) {
        const movs = await deducirStockFIFO(tx, ingredienteId, cantidadTotal, pedido.id);
        movimientosFIFO.push(...movs);
      }

      // Crear movimientos FIFO en batch
      if (movimientosFIFO.length > 0) {
        await tx.movimientoStock.createMany({ data: movimientosFIFO });
      }
...
const cancelarPedido = async (prisma, payload) => {
  const { pedidoId, motivo, usuarioId } = payload;

  const result = await prisma.$transaction(async (tx) => {
    const pedido = await tx.pedido.findUnique({
      where: { id: pedidoId },
      include: { movimientos: true, items: { include: { producto: { select: { nombre: true } } } } }
    });
    ...
    if (pedido.estado !== 'PENDIENTE') {
      ...
      if (salidaMovements.length > 0) {
        ...
        stockRevertido = salidaMovements.map(mov => ({
          ingredienteId: mov.ingredienteId,
    ...
    if (usuarioId) {
      await tx.auditoriaAnulacion.create({
        data: {
          tipo: 'CANCELACION_PEDIDO',
          pedidoId,
          usuarioId,
          motivo: motivo || null,
          montoAfectado: pedido.total,
          detalles: {
