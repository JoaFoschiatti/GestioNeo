...
async function processIncomingTransfer(movementData) {
  ...
  if (matchResult.matched && matchResult.pedidoId) {
    ...
    eventBus.publish('transferencia.pendiente', {
      transferencia,
      matchResult
    });
...
async function executeMatch(transferenciaId, pedidoId, score, reason) {
  return prisma.$transaction(async (tx) => {
    ...
    eventBus.publish('pedido.updated', {
      pedidoId,
      estadoPago: 'APROBADO',
      estado: pedidoTotalmentePagedo ? 'COBRADO' : pedido.estado
    });
    eventBus.publish('transferencia.matched', { transferenciaId, pedidoId });
