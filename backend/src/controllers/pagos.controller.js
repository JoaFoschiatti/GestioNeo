...
const DEFAULT_WEBHOOK_TS_TOLERANCE_SECONDS = 300;

const getWebhookTsToleranceSeconds = () => {
  const parsed = parseInt(
    process.env.MERCADOPAGO_WEBHOOK_TS_TOLERANCE_SECONDS || `${DEFAULT_WEBHOOK_TS_TOLERANCE_SECONDS}`,
    10
  );
  if (Number.isNaN(parsed) || parsed < 30) {
    return DEFAULT_WEBHOOK_TS_TOLERANCE_SECONDS;
  }
  return parsed;
};

const isFreshWebhookTimestamp = (tsRaw) => {
  ...
  const tolerance = getWebhookTsToleranceSeconds();
...
const registrarPago = async (req, res) => {
  ...
  const { pedidoId, monto, metodo, referencia, comprobante, idempotencyKey, propina } = req.body;
...
const verifyWebhookSignature = (req) => {
  ...
  if (!isFreshWebhookTimestamp(ts)) {
    logger.warn('Webhook MercadoPago rechazado por timestamp fuera de tolerancia', { ts });
    return false;
  }
...
const webhookMercadoPago = async (req, res) => {
  try {
    ...
    if (!module.exports._verifyWebhookSignature(req)) {
    ...
    if (type === 'payment' || action === 'payment.created' || action === 'payment.updated') {
      ...
      try {
        ...
        logger.error('Error al consultar pago con credenciales configuradas:', mpError);
        paymentInfo = await getPaymentFromGlobalToken(paymentIdStr);
      }

      if (!paymentInfo) {
        logger.error('No se pudo obtener info del pago desde MercadoPago');
        return res.sendStatus(200);
      }
      ...
      // Parsear external_reference para obtener pedidoId
      // Formatos soportados: "1-{pedidoId}", "pedido-{pedidoId}", "{pedidoId}"
      if (!pedidoId && paymentInfo.external_reference) {
        const ref = paymentInfo.external_reference.toString();
        const numericMatch = ref.match(/^(\d+)-(\d+)$/);
        const legacyMatch = ref.match(/^pedido-(\d+)$/);

        let parsedPedidoId;
        if (numericMatch) {
          parsedPedidoId = parseInt(numericMatch[2], 10);
        } else if (legacyMatch) {
          parsedPedidoId = parseInt(legacyMatch[1], 10);
        } else {
          parsedPedidoId = parseInt(ref, 10);
        }
        ...
        if (!Number.isNaN(parsedPedidoId)) {
          ...
          if (pedido) {
            ...
            logger.warn('Webhook: Pedido no encontrado', { pedidoId: parsedPedidoId });
          }
        }
      }
      ...
      // Si no encontramos pago por mpPaymentId/idempotencyKey, buscar por preferenceId o pedidoId
      if (!pagoExistente) {
      ...
      if (estadoPago === 'APROBADO') {
        ...
        // Completar pedido y liberar mesa si el total está cubierto
        const { pedidoActualizado, mesaLiberada, mesaId } =
          await pagosService.completarPagoPedido(prisma, pedidoId);

        eventBus.publish('pedido.updated', {
          id: pedidoActualizado?.id || pedido.id,
          estado: pedidoActualizado?.estado || pedido.estado,
          estadoPago: 'APROBADO',
          tipo: pedido.tipo,
          mesaId: pedido.mesaId || null,
        ...
        if (pedido.clienteEmail) {
          try {
            ...
            await emailService.sendOrderConfirmation(pedido, negocio);
...
module.exports = {
  ...
  _verifyWebhookSignature: verifyWebhookSignature
};
