...
const webhookSuscripcion = async (req, res) => {
  ...
  try {
    // Verificar firma si está configurada (en todos los entornos, no solo producción)
    const webhookSecret = process.env.MP_SUBSCRIPTION_WEBHOOK_SECRET;
    const isPlaceholder = !webhookSecret || webhookSecret === 'your_subscription_webhook_secret_here';
    ...
    if (!isPlaceholder) {
      ...
      if (!xSignature) {
        logger.warn('Webhook de suscripción sin firma');
        return responder200();
      }
      ...
      if (!ts || !v1) {
        logger.warn('Webhook de suscripción: firma incompleta');
        return responder200();
      }
      ...
      try {
        ...
        if (expected.length !== actual.length || !crypto.timingSafeEqual(expected, actual)) {
          logger.warn('Webhook de suscripción: firma inválida');
          return responder200();
        }
      } catch (err) {
        logger.warn('Webhook de suscripción: error verificando firma', err);
        return responder200();
      }
    }
    ...
    if (!type || !data?.id) {
      logger.warn('Webhook de suscripción: payload incompleto', req.body);
      return responder200();
    }
    ...
    setImmediate(async () => {
      try {
        ...
        logger.error('Error procesando webhook de suscripción:', error);
      }
    });
