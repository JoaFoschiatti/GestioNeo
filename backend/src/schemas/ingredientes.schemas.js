...
const registrarMovimientoBodySchema = z.object({
  ...
  categoriaGasto: z.preprocess((val) => (val === '' ? null : val), z.string().nullable().optional()),
  costoUnitario: z.preprocess((val) => (val === '' ? null : val), z.coerce.number().min(0).nullable().optional())
}).strip();
...
const crearLoteBodySchema = z.object({
  cantidad: z.coerce.number().positive(),
  codigoLote: z.preprocess((val) => (val === '' ? null : val), z.string().nullable().optional()),
  costoUnitario: z.preprocess((val) => (val === '' ? null : val), z.coerce.number().min(0).nullable().optional()),
  fechaVencimiento: z.preprocess((val) => (val === '' ? null : val), z.string().nullable().optional()),
  categoriaGasto: z.preprocess((val) => (val === '' ? null : val), z.string().nullable().optional())
}).strip();

const alertasVencimientoQuerySchema = z.object({
  dias: z.coerce.number().int().min(1).max(365).optional()
}).strip();

const loteIdParamSchema = z.object({
