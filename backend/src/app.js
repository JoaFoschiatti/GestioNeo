...
if (trustProxyRaw === 'true') {
  ...
  // Production usually runs behind a reverse proxy/load balancer.
  app.set('trust proxy', 1);
}
...
app.use(cors({
  origin: (origin, callback) => {
    // En producción, rechazar requests sin Origin (previene CSRF via form submissions)
    // En desarrollo, permitir requests sin Origin (Postman, curl, mobile apps)
    if (!origin) {
      if (process.env.NODE_ENV === 'production') {
