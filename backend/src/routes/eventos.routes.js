...
router.get('/', (req, res, next) => {
  // Prioridad de auth: 1) cookie httpOnly (seguro), 2) Authorization header, 3) query param (solo SSE de 30s)
  // Si ya hay cookie o header, no necesitamos el query param
  const token = req.query.token;
  if (token && !req.headers.authorization && !req.cookies?.token) {
