...
router.get('/lotes/alertas-vencimiento', validate({ query: alertasVencimientoQuerySchema }), asyncHandler(ingredientesController.alertasVencimiento));
router.get('/:id', validate({ params: idParamSchema }), asyncHandler(ingredientesController.obtener));
router.get('/:id/lotes', validate({ params: idParamSchema }), asyncHandler(ingredientesController.listarLotes));
...
router.post(
  ...
  bloquearSiSoloLectura,
  esAdmin,
  validate({ params: idParamSchema, body: crearLoteBodySchema }),
  asyncHandler(ingredientesController.crearLote)
);
router.post(
  '/lotes/:loteId/descartar',
  bloquearSiSoloLectura,
  esAdmin,
  validate({ params: loteIdParamSchema }),
