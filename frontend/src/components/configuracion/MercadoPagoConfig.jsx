...
export default function MercadoPagoConfig({ onStatusChange }) {
  ...
  const handleConnectOAuth = async () => {
    try {
      ...
      const data = response.data
      // Validar que la URL apunte a MercadoPago antes de redirigir
      try {
        const url = new URL(data.authUrl)
        if (!url.hostname.endsWith('mercadopago.com') && !url.hostname.endsWith('mercadopago.com.ar')) {
          throw new Error('URL de OAuth no apunta a MercadoPago')
        }
      } catch (urlErr) {
