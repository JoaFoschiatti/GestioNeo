...
export default function Productos() {
  ...
  const shortcutsList = useMemo(() => [
    { key: 'N', description: 'Nuevo producto' },
    { key: 'Esc', description: 'Cerrar modal' },
    { key: '?', description: 'Ayuda de atajos' },
  ], [])

  useKeyboardShortcuts(useMemo(() => ({
    'n': () => { resetForm(); setShowModal(true) },
    'Escape': () => {
      if (showShortcutsHelp) setShowShortcutsHelp(false)
      else if (showAgruparModal) { setShowAgruparModal(false); resetAgruparForm() }
      else if (showVarianteModal) { setShowVarianteModal(false); resetVarianteForm() }
      else if (showModal) setShowModal(false)
    },
    '?': () => setShowShortcutsHelp(prev => !prev),
  }), [showShortcutsHelp, showAgruparModal, showVarianteModal, showModal]))
