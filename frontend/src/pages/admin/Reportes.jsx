...
// Componente: Tabla de Auditoria de Anulaciones
const AuditoriaAnulacionesTable = ({ data, loading }) => {
  if (loading) {
    return <p className="text-text-secondary text-center py-4">Cargando...</p>
  }
  if (!data || data.length === 0) {
    return <p className="text-text-secondary text-center py-4">No hay anulaciones en el periodo seleccionado</p>
  }
  ...
  const tipoLabels = {
    CANCELACION_PEDIDO: 'Cancelacion',
    ANULACION_ITEM: 'Anulacion Item',
        ...
        <tbody>
          {data.map((item) => (
            <tr key={item.id} className="hover:bg-surface-hover">
              <td className="text-sm text-text-secondary">{formatFecha(item.createdAt)}</td>
              <td>
                <span className="badge badge-error text-xs">
                  {tipoLabels[item.tipo] || item.tipo}
                </span>
              </td>
              <td className="text-sm text-text-primary">{item.usuario?.nombre || '-'}</td>
              <td className="text-sm text-text-secondary">
                {item.pedido ? `#${item.pedido.id} (${item.pedido.tipo})` : '-'}
              </td>
              <td className="text-sm text-text-secondary max-w-48 truncate">{item.motivo || 'Sin motivo'}</td>
              <td className="text-right text-sm font-medium text-error-600">
                {item.montoAfectado ? `$${Number(item.montoAfectado).toLocaleString('es-AR')}` : '-'}
              </td>
            </tr>
          ))}
        </tbody>
...
export default function Reportes() {
  ...
  const [auditoriaAnulaciones, setAuditoriaAnulaciones] = useState([])
  ...
  // Cargar gastos cuando se selecciona el tab
  useEffect(() => {
    if (tabActiva === 'gastos' && fechaDesde && fechaHasta) {
      setLoadingGastos(true)
      api.get(`/reportes/gastos?fechaDesde=${fechaDesde}&fechaHasta=${fechaHasta}`, { skipToast: true })
        .then(res => setGastosData(res.data))
        .catch(err => {
  ...
  }, [tabActiva, fechaDesde, fechaHasta])

  // Cargar auditoria cuando se selecciona el tab
  useEffect(() => {
    if (tabActiva === 'auditoria' && fechaDesde && fechaHasta) {
      setLoadingAuditoria(true)
      api.get(`/reportes/auditoria-anulaciones?fechaDesde=${fechaDesde}&fechaHasta=${fechaHasta}`, { skipToast: true })
        .then(res => setAuditoriaAnulaciones(res.data))
        .catch(err => {
          console.error('Error auditoria:', err)
          setAuditoriaAnulaciones([])
        })
    ...
    <div>
      ...
      <div className="tabs mb-6">
        ...
        <button
          ...
          className={`tab ${tabActiva === 'gastos' ? 'tab-active' : ''}`}
        >
          Gastos
        </button>
        <button
        ...
        <div className="space-y-6">
          {loadingGastos ? (
            <div className="card text-center py-8">
              <p className="text-text-secondary">Cargando gastos...</p>
            </div>
          ) : gastosData ? (
            <>
              {/* KPIs de gastos */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="card">
                  ...
                  <p className="text-2xl font-bold text-error-600">
                    ${gastosData.totalGastos?.toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                  </p>
                </div>
                <div className="card">
                  <p className="text-sm text-text-secondary">Compras Registradas</p>
                  <p className="text-2xl font-bold text-text-primary">{gastosData.totalMovimientos}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Donut por categoria */}
                <div className="card">
                  <h3 className="text-heading-3 mb-4">Gastos por Categoria</h3>
                  {gastosData.porCategoria && gastosData.porCategoria.length > 0 ? (
                    <DonutChart
                      data={gastosData.porCategoria.map(c => ({
                        name: c.categoria,
                        value: c.total
                      }))}
                      colors={['#ef4444', '#f97316', '#eab308', '#22c55e', '#06b6d4', '#8b5cf6', '#6b7280']}
                      formatValue={(v) => `$${v.toLocaleString('es-AR', { minimumFractionDigits: 2 })}`}
                    />
                  ) : (
                    <p className="text-text-secondary text-center py-8">Sin datos de gastos</p>
                  )}
                </div>

                {/* Tabla detalle */}
                <div className="card">
                  <h3 className="text-heading-3 mb-4">Detalle por Categoria</h3>
                  {gastosData.porCategoria && gastosData.porCategoria.length > 0 ? (
                    <div className="space-y-3">
                      {gastosData.porCategoria.map((cat) => (
                        <div key={cat.categoria} className="bg-surface-hover p-3 rounded-lg">
                          <div className="flex justify-between items-center mb-1">
                            <span className="font-medium text-text-primary">{cat.categoria}</span>
                            <span className="font-bold text-error-600">
                              ${cat.total.toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                            </span>
                          </div>
                          <p className="text-xs text-text-tertiary">{cat.cantidad} compras</p>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-text-secondary text-center py-4">Sin datos</p>
                  )}
                </div>
              </div>
            </>
            ...
            <div className="card text-center py-8">
              <p className="text-text-secondary">Selecciona un rango de fechas y presiona Actualizar</p>
            </div>
          )}
        </div>
      )}
      ...
      {tabActiva === 'auditoria' && (
        <div className="card">
          <h3 className="text-heading-3 mb-4">Auditoria de Anulaciones</h3>
          <AuditoriaAnulacionesTable data={auditoriaAnulaciones} loading={loadingAuditoria} />
        </div>
      )}
    </div>
  )
