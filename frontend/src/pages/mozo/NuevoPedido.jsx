  ...
  const enviarPedido = async () => {
    ...
    try {
      ...
      const response = await api.post('/pedidos', pedidoData, {
        ...
        offlineCapable: true,
        _tempId: tempId,
        _optimisticResponse: { id: tempId, estado: 'PENDIENTE', ...pedidoData, _offline: true },
        _offlineDescription: `Crear pedido ${tipo === 'MESA' ? `Mesa ${mesa?.numero}` : 'Delivery'}`
      })

      if (response._offline) {
        toast.success('Pedido guardado. Se enviara al servidor cuando haya conexion.')
      } else {
